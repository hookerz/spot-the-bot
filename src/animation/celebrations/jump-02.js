...
export default function ( mesh, bones, onComplete ) {
  ...
  const tl = new TimelineMax ( { repeat: 0, onComplete: onComplete } );
        ...
        */
        ...
        tl.to ( root.rotation, 1.2, { y: Math.PI * 4.0, ease: eases.d }, 'lift+=0.0' );
        ...
        tl.add ((() => {
          ...
          const tl = new TimelineMax ();
                ...
                tl.fromTo ( positions, 0.4, { y: 0.0, immediateRender: true }, { y: spread * 0.6, ease: Power3.easeIn } );
                tl.to ( positions, 0.8, { y: spread * 1.0, ease: Elastic.easeOut.config ( 1.2, 0.8 ) }, 0.3 );
        ...
        // let's just doing a shrinky dink then

        tl.add ( 'shrank', '-=0.6' );

        tl.add([

          //TweenMax.to ( positions, 0.2, { y: spread * 1.2, ease: Power2.easeOut } ),
          TweenMax.to ( positions, 0.6, { y: spread * 0.0, ease: Power2.easeIn }, 0.06 ),

        ], 'shrank+=0.6', 'sequence' );

        tl.add([

          TweenMax.to ( root.rotation, 0.2, { y: '+=0.2', ease: Power1.easeOut } ),
          TweenMax.to ( root.rotation, 2.0, { y: -Math.PI * 12.0, ease: Power1.easeIn } ),

        ], 'shrank+=0.0', 'sequence' );

        tl.add ([

          TweenMax.to ( root.scale, 0.4, { x: 0.0, y: 0.1, z: 0.0, ease: Power4.easeIn } )
