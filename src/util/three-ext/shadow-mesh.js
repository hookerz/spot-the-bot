...
export const ShadowMesh = function ( mesh, shadowMaterial ) {

	 shadowMaterial = shadowMaterial || new MeshBasicMaterial( {
		color: 0x000000,
		transparent: true,
		opacity: 0.6,
		depthWrite: false
...
ShadowMesh.prototype.update = function () {
	...
	return function ( plane, lightPosition4D ) {
		...
		let dot = plane.normal.x * lightPosition4D.x +
			  plane.normal.y * lightPosition4D.y +
			  plane.normal.z * lightPosition4D.z +
			  - plane.constant * lightPosition4D.w;

		let sme = shadowMatrix.elements;
